...
def show_subject_graph():
    ...
    hhi_cache = detector.group_hhi_cache if detector else {}
    ...
    with col2:
        # 显示当前群的HHI指数
        hhi_value = hhi_cache.get(selected_group, 0.0)
        hhi_percent = hhi_value * 100
        ...
        st.metric(
            label="HHI 赫芬达尔指数",
            value=f"{hhi_value:.4f}",
            delta=f"{hhi_level} {hhi_color}",
            delta_color="off"
        )
