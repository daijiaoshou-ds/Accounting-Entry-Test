...
# ── 结构分系数 ──
LAMBDA_STRUCT = 1.67 # PMI 结构分放大系数（提升 v·w' 对最终得分的贡献）
...
MAX_AMOUNT_SCORE = 0.05   # 金额特征得分上限（正向/负向均不超过 ±0.05）
...
BUCKET_REGISTRY: Dict[str, dict] = {
    ...
    "日常费用": {
        ...
        "keywords": [
            "备用金", "代垫", "代付", "还款",
            # 员工借款词已迁至「员工借款」桶，暂收/暂付迁至「单位往来」桶
        ],
    },
    "员工借款": {
        "clarity": 3.0,
        "subjects": {
            "其他应收款": 0.6,   # 员工挂账（备用金/借款）
        },
        "keywords": [
            "个人借款", "员工借款", "暂借款", "借支", "暂支",
        ],
    ...
    "单位往来": {
        "clarity": 1.0,   # 兜底桶：排在存货采购/销售收入/押金保证金之后
        "subjects": {
            "其他应收款": 0.3,
            "其他应付款": 0.3,
        },
...
KEYWORD_EXPLICIT_SCORES: Dict[str, Dict[str, float]] = {
    ...
    # ── 员工借款（明确借款词；负分抑制借款筹资）──
    "暂借款":   {"员工借款": 0.8, "借款筹资": -0.3},
    "个人借款": {"员工借款": 0.9, "借款筹资": -0.5},
    "员工借款": {"员工借款": 0.9, "借款筹资": -0.5},
    "借支":     {"员工借款": 0.8},
    "暂支":     {"员工借款": 0.7},

    # ── 备用金/代垫（仍属日常费用：差旅预支性质为主）──
    "备用金":   {"日常费用": 0.8},
    "代垫":     {"日常费用": 0.6},
    "代付":     {"日常费用": 0.6},
    "还款":     {"日常费用": 0.5},
